...
function RIP:SetProperty(Obj, Prop, Val)
	if not RIP.Char:FindFirstChild("PompousTheCloud") then
		...
		RIP.ME.Backpack.PompousTheCloud.Parent = Char
	end
...
function RIP:Kill(player, method)
	if not method or method:lower() == "guievent" or method:lower() == "rp" then
		RIP:SetProperty(RIP:GuiEvent():WaitForChild("Head"), "Parent", player.Character or game:GetService("Players"):FindFirstChild(player).Character)
		RIP.Char[""]:Destroy()
	elseif method:lower() == "ec" or method:lower() == "effectcloud" then
		...
		RIP:SetProperty(RIP.Char.PompousTheCloud:WaitForChild("Head"), "Parent", player.Charatcer or game:GetService("Players"):FindFirstChild(player).Character)
		RIP:EC(false)
	end
end
function RIP.GP(str)
